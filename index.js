...
(function(){
  var fs, path, _, crypto, Promise, OpenCC, opencc, opts, md5, filepath, find, overwrite, mkdirsIfNotOverwrite, pwd, fileList, dirHead, ignore, ig;
  fs = require('fs');
  path = require('path');
  ...
  Promise.map(fileList, function(file){
    return fs.readFileAsync(file, 'utf8').then(function(data){
      var hash, translateData, translateHash, result;
      hash = md5(data);
      ...
      if (result.required || !opts.overwrite) {
        console.log('writing file: ' + path.join(pwd, file) + (result.required ? '(changed)' : ''));
        return fs.writeFileAsync(path.join(pwd, file), result.data);
      }
    });
  }).then(function(){
